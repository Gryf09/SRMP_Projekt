% Smallest of averaging CFAR 2D
function [noise_est,det_vect] = so_ca_cfar_2D(x, N_ref, N_guard, T)
    ...
    N = sum([1:2*N_guard + 2*N_ref + 1]) - sum([1:2*N_guard + 1]) ; % liczba komórek testowanych
    for m = N_guard + N_ref + 1 : length(x) - N_guard - N_ref
       for n = N_guard + N_ref + 1 : length(x) - N_guard - N_ref
        ...
        mean_r = 0;
           ...
           % MIN
           noise_est(m,n) = min(mean_l,mean_r);
